...
users.userRegistration = async function (req, res) {
      ...
      if (userData.message) {
        ...
        if (result.length > 0) {
          ...
          let mailOptions = {
            ...
            html: '<h1>Email verification lik<h1>: <a href="http://50.28.103.83:4001/register-confirmation/' + userData.verification_token +' ">verify email</a>'
          }
  ...
  users.updateProfile = async (req, res) => {
    try {
      let { userId } = req.user;
      let { image, gendar, last_name, phone, first_name, city, dob_date, proffesion,zipcode } = req.body;
      let profile_img = "";
      user.findOne({
        where: {
          id: userId
